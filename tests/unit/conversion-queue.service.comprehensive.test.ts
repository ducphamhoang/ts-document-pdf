...
describe('ConversionQueueService Comprehensive Tests', () => {
  ...
  describe('Queue behavior', () => {
    ...
    it('should maintain queue order (FIFO)', async () => {
      ...
      try {
        ...
        const task1Promise = new Promise<void>(resolve => setTimeout(() => {
          executionOrder.push(1);
          resolve();
        }, 10));
        const task1 = jest.fn().mockImplementation(() => task1Promise);
        
        const task2Promise = new Promise<void>(resolve => setTimeout(() => {
          executionOrder.push(2);
          resolve();
        }, 10));
        const task2 = jest.fn().mockImplementation(() => task2Promise);
        
        const task3Promise = new Promise<void>(resolve => setTimeout(() => {
  ...
  describe('Error handling', () => {
    it('should propagate errors from tasks executed immediately', async () => {
      ...
      await expect(queueService.execute(errorTask))
        .rejects
        .toThrow('Task failed');
    ...
    it('should propagate errors from queued tasks', async () => {
      ...
      try {
        ...
        await expect(queuedPromise)
          .rejects
          .toThrow('Queued task failed');
