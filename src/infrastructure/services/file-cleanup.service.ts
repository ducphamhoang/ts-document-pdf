  ...
  async cleanupStaleTempDirectories(): Promise<void> {
    ...
    try {
      ...
      const staleDirs = files.filter(file => {
        return file.startsWith('convert-') && file.length > 'convert-'.length + 5; // Make sure it's a proper convert-XXXXX directory
      });
      ...
      logger.info(
        `Completed cleanup of stale temporary directories. Processed ${staleDirs.length} directories.`
      );
    } catch (error) {
